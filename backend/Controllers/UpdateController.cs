...
namespace backend.Controllers
{
    ...
    public async Task<ActionResult> UploadRemnantUpdates([FromBody] RemnantUpdateRequest request)
    {
        try
        {
            ...
            // КОНВЕРТАЦИЯ: преобразуем DTO в Entity
            var remnantUpdates = request.ArrayOfRemnantsEl.Select(dto => new RemnantUpdate
            {
                ID = 0, // Auto-generated by DB
                ProductId = dto.ID, // Используем ID из JSON как ProductId
                StockId = dto.IDStock, // Используем IDStock из JSON как StockId
                InStockT = dto.InStockT,
                InStockM = dto.InStockM,
                SoonArriveT = dto.SoonArriveT,
                SoonArriveM = dto.SoonArriveM,
                ReservedT = dto.ReservedT,
                ReservedM = dto.ReservedM,
                AvgTubeLength = dto.AvgTubeLength,
                AvgTubeWeight = dto.AvgTubeWeight,
                UpdatedAt = DateTime.UtcNow,
                IsProcessed = false
            }).ToList();

            var result = await _updateService.ProcessRemnantUpdatesAsync(remnantUpdates);
